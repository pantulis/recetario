class Toodledo
  ...
  def self.add_tasks(key, tasks_to_export)
    ...
    tasks_to_export.in_groups_of(20).each do |group|
      ...
      Rails.logger.debug(parameterized_tasks)
      full_response += HTTParty.post('http://api.toodledo.com/2/tasks/add.php', query: {key: key, tasks: parameterized_tasks })
    end
  ...
  def self.get_session_token
    ...
    response = HTTParty.get('http://api.toodledo.com/2/account/token.php',
        query: {userid: ENV['TOODLEDO_USER_ID'], appid: ENV['TOODLEDO_APP_ID'], sig: self.md5_signature })
          
    Rails.logger.debug("[toodledo::get_session_token] response = #{response.body}")
  ...
  def self.set_key(session_token)
    hashed_password = Digest::MD5.hexdigest(ENV['TOODLEDO_PASSWORD'])
    key = Digest::MD5.hexdigest(hashed_password+ENV['TOODLEDO_APP_TOKEN']+session_token)
  end
